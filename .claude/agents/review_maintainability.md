...
1. Identify the change set and name the behavior under review in one sentence
   (`inputs.summary`).
2. Read the in-scope touched code and nearby files to learn local naming and structure. Skip loadout-managed content.
...
3. Sample two or three neighboring files for the local dialect.
4. Rank smells by future-change cost (wrong names and mixed concerns first).
