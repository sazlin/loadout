...
def _reset_to_head(path: Path, head: str) -> None:
    _run_git(["-C", str(path), "reset", "--hard", "--", head])
...
def _sha_on_branch(sha: str, branch: str) -> bool:
    contained = _run_git(["merge-base", "--is-ancestor", "--", sha, branch])
...
def _cmd_cherry_pick(args: list[str]) -> int:
    ...
    if not _sha_on_branch(sha, branch):
