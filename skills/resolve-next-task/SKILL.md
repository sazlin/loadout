...
description: >-
  Implement the first open hashed-tasks-file task, commit, and push to the
  PR branch. Use when issue_resolver is dispatched, or when the user says
...
1. Read `tasks_path` from the brief (`TASKS_TO_RESOLVE-<short-sha>.md`). If
   ...
   match. If there is no `open` task, stop and report done (nothing to
   resolve).
