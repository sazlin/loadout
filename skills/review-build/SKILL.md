...
name: review-build
description: >-
  Use when imp_builder needs a fresh imp_reviewer to critique the
  implementation against IMPLEMENTATION_PLAN.md and the PRD. Do not edit
  product code in-process.
---

# Review build

Dispatch `imp_reviewer` on a **fresh** subagent to review the current tree
against the plan and PRD.

## When to use

- `imp_builder` finished a build or a revision
- The factory asks for `/review-build`
...
2. Dispatch **one** isolated `imp_reviewer` subagent with: "Follow
   `.claude/agents/imp_reviewer.md`. Return only your JSON issue schema.
   Do not edit files. Do not fix the code."
