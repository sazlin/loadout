...
If you don’t have a Stripe account, the user can generate working API keys with no registration by installing the Stripe CLI themselves and running `stripe sandbox create`. Point them at [Stripe CLI install](https://docs.stripe.com/stripe-cli). Do not run `npm i -g`, `npx`, `curl | sh`, Homebrew, or other unpinned installs to obtain the CLI or keys. If `stripe` is missing, stop and tell the user how to install it.
