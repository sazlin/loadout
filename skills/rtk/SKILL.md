...
name: rtk
description: >-
  Use when running noisy CLI commands (git status, git diff, git log, pytest,
  cargo test, npm test, ls, docker ps, ruff, grep) in an agent session, when
  bash output is eating context, or when the user mentions rtk, Token Killer,
  or token compression for command output.
license: Apache-2.0
---
