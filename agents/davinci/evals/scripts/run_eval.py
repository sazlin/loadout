...
def load_evals() -> list[EvalCase]:
    ...
    for entry in data["evals"]:
        ...
        name = entry.get("name")
        if not isinstance(eval_id, int) or not isinstance(name, str):
            continue
...
def build_davinci_prompt_cloud(case: EvalCase, source: str) -> str:
    ...
    return (
        ...
        f"---- CHARTER (agents/davinci/davinci.md) ----\n{charter}\n---- END CHARTER ----\n\n"
