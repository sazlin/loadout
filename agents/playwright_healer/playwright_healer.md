...
2. On failure, open the page with `npx playwright-cli`, `snapshot` the UI, and inspect locators (`npx playwright-cli generate-locator <ref>`), `console`, and `requests`. Use `npx playwright show-trace` when a trace exists. Do not use `npx playwright test --debug` unless a human is watching a headed session.
